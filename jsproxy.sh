...
nginx_conf="${nginx_conf_dir}/jsproxy.conf"
...
show_information(){
    ...
    echo -e "${OK} ${GreenBG} jsproxy 配置信息 ${Font}"
    echo -e "${OK} ${GreenBG} 地址（address）:${Font} ${domain} "
    echo -e "${OK} ${GreenBG} 端口（port）：${Font} ${port} "
}
...
nginx_modify(){
      ...
      tar zxf www.tar.gz -C $JSPROXY_VER/www --strip-components=1
      rm -f www.tar.gz
}
